...
// COULD MAKE THIS HAVE QUEUE IMPLEMENTATION!
public class DropdownController : VariableControllerInput
{
    ...
    private void AnyDropdownChange(TMP_Dropdown dropdown, List<string> availableOptions, int i)
    {
        ...
        GameControl.control.selectedVariables[i] = dropdown.options[dropdown.value].text;
        GameControl.control.unselectedVariables[0] = availableOptions[1];
        GameControl.control.unselectedVariables[1] = availableOptions[2];

        GameControl.control.textController.UpdateVariableText();
        // call function to activate the inputfield for the variable selected
        base.SetInputActive(i, dropdown.options[dropdown.value].text);
    }
